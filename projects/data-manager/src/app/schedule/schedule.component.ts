...
import { SchedulePopupComponent } from './log-popup/log-popup.component';
...
export class ScheduleComponent implements OnInit {
  ...
  // o_displayColNms: string[] = ['스케줄명', '워크플로우명', '입력 데이터', '출력 데이터', '상태', '시작 날짜', '종료 날짜', '주기', '등록 날짜'];
  // o_displayCols: string[] = ['SCH_NM', 'WF_NM', 'INPUT_DATA', 'OUTPUT_DATA', 'SCH_STATUS', 'ST_DT', 'ED_DT', 'CRON_INFO', 'REG_DT'];
  o_gridCol: any;
  o_gridRowEvt = true;
  o_gridHeader = { btnNmList: [this.cTransSvc.instant("WPP_DATA_MANAGER.SCHEDULE.BUTTON.button1")], 
    filterCol: ['SCH_NM'] };
  // WPLAT-345 COMMON일 경우
  // o_gridHeader = { btnNmList: [this.cTransSvc.instant("WPP_DATA_MANAGER.SCHEDULE.BUTTON.button1")], filterCol: ['SCH_NM'] };
  o_ScheduleForm: any[];
  o_realtimeForm: any[];
  ...
  openLog(p_item: any) {
    this.cWpLibSvc.showProgress(true, "wdspin");
    const dialogRef = this.cDialog.open(SchedulePopupComponent, {
