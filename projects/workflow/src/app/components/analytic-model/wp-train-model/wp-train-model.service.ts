...
export class WpTrainModelService extends WpSeriveImple {
    constructor(
    ...
    // YOLO
    o_metricsGridCol = [
        { NAME: 'varNm', VISIBLE: true, VNAME: '평가지표 명', TYPE: 'string' },
        { NAME: 'importance', VISIBLE: true, VNAME: '값', TYPE: 'number' },
    ]
    // 클러스터 차트 색상
    h_ChartColors = [
        '#577df6',
    ...
    getModelResultGridData(p_modelResult: any) {
        ...
        // 변수 영향도 그리드 데이터
        if (s_modelType !== 'Clustering' && s_modelType != 'Image'  && s_evalResult.hasOwnProperty('featureLog') && s_evalResult.featureLog !== '') {
        ...
        else if (s_modelType == 'Classification' || s_modelType == 'Image') {
        ...
        let sResult: any = {
            ...
            metricsGridCol: this.o_metricsGridCol, metricsGridData: s_metricsGridData
        }
        if (s_modelType == 'Regression') {
    ...
    getModelChartData(p_modelResult: any) {
        ...
        // 분류 차트 데이터 280강화학습
        if (s_modelType == 'Classification' || s_modelType == 'Reinforcement' || s_modelType == 'Image') {
            let s_modelResult = JSON.parse(s_evalResult.evaluateLog.result);
