...
import { COM_TRANSFER_ATT } from "projects/wp-server/wp-type/WP_COM_ATT";

import { TranslateService } from "@ngx-translate/core";
import { WpTransferModelService } from "./wp-transfer-model.service";
import { WpTransferModelData } from "projects/wp-server/util/component/analytic/wp-transfer-model";
import { WpPythonPopupComponent } from "../../popup/wp-python-popup.component";
import { MatDialog } from '@angular/material/dialog';
export class WpTransferModelComponent extends WpComponent {
    oWpData: COM_TRANSFER_ATT;
    oWpTransferModelSvc: WpTransferModelService;
    ...
    o_wppModelList:any = [];
    h_predictModelList:any = [];
    o_dialog: MatDialog;
    constructor(
        pTransSvc: TranslateService,
        pComViewerSvc: WpComponentViewerService,
        pWpData: WpTransferModelData,
        pWpTransferModelSvc: WpTransferModelService,
        ...
        this.oWpTransferModelSvc = pWpTransferModelSvc;
        ...
        this.oModelFormData = [
            ...
            },
            {
                ...
                value: '',
                type: 'select',
                fvalue: ['Classification', 'Regression'],
                visible: true,
                edit: true,
        ...
        if (pTabName == 'Info') {
            ...
            this.oWpDiagramSvc.getModelResult([this.oComId]).then((pResult: any) => {
                if (pResult.length > 0) {
                    ...
                    let sResult = this.oWpTransferModelSvc.getModelResultGridData(s_modelResult);
                    if (sResult.gridCol.length > 0 && sResult.gridData) {
                        // 모델 정보 form value
                        let sModelType = s_modelResult.argInfo.ARG_TYPE;
                        let sModelTypeKor = sModelType === 'Classification' ? this.cTransSvc.instant("WPP_WORKFLOW.COMPONENT.INFO.info80") : sModelType === 'Regression' ? this.cTransSvc.instant("WPP_WORKFLOW.COMPONENT.INFO.info91") : this.cTransSvc.instant("WPP_WORKFLOW.COMPONENT.INFO.info90");
                        let sModelInfo = [
                        ...
                        // 차트 데이터 설정
                        this.oChartData = this.oWpTransferModelSvc.getModelChartData(s_modelResult);
    ...
    async setModelInfo(p_argInfo: any) {
        ...
        this.o_argParam = await this.oWpTransferModelSvc.getArgParam(p_argInfo);
    ...
    public async onArgParamChanged(p_ev: any, p_form: any, p_idx: any = null) {
        ...
        console.log("this.oWpData.parameter : ", this.oWpData.parameter);
    }
    ...
    async isValidExtra(p_value: any, p_form: any, p_formname: string) {
        ...
        if (p_formname == 'parameter') {
            ...
            let s_checkModelName: any = await this.oWpTransferModelSvc.checkModelName(p_value);
    ...
    public async onArgFormChanged(p_ev: any, p_form: any) {
        ...
        if (s_formName == 'modelSave') {
            ...
            if(s_overwriteFlag == true && this.o_overwriteList == null) {
                this.o_overwriteList = await this.oWpTransferModelSvc.getOverwriteModelList();
            }
        ...
        }  else if (s_formName == 'argType') {
            this.o_argType = s_formValue;
            this.oWpData.target_column = '';
            let s_argId = '4000';
            if(s_formValue == 'Regression') {
                s_argId = '4001';
            }
            let s_argInfo:any = await this.oComViewerSvc.getArgInfo(s_argId);
            console.log("s_argInfo: ", s_argInfo);
            this.oWpData.modelInfo['model'] = s_argInfo[0];
            await this.setModelInfo(s_argId);
        }
    ...
    async getModelList() {
        // 기존 모델 리스트
        await this.oWpTransferModelSvc.getModelList().toPromise().then((pData: any) => {
            // 실행결과가 존재하는 모델만 비교 가능
            if (pData.result.length > 0) {
                pData.result.forEach((sModel:any) => {
                    this.o_customModelList = pData.result.filter((model:any) => model.CUSTOM_YN === 'Y');
                    this.o_wppModelList = pData.result.filter((model:any) => model.CUSTOM_YN === 'N');
                });
            }
            ...
            this.oFormData.forEach(sForm => {
                if (sForm.name == 'modelName') {
                    sForm.fvalue = this.h_predictModelList.map((sModel:any) => sModel.MODEL_NM);
                }
            });
        });
    }

    async onChangeModel(p_ev: any, p_form:any) {
       
        // this.oComViewerSvc.showProgress(true);
        // let sSelectIndex = this.getItemIndexByElem(pEvent.component._valueChangeEventInstance.currentTarget);
        let sSelectModel = this.h_predictModelList.find((model:any) => model.MODEL_NM === p_ev.selectedItem);
        console.log("sSelectModel : ", sSelectModel);
        if(this.oWpData.modelName == '' || (this.oWpData.modelName != sSelectModel.MODEL_NM)) {
            this.oWpData.pythonCode = '';
            this.oWpData.modelName = sSelectModel.MODEL_NM;
            this.oWpData.learnedModel = sSelectModel;
            this.oWpData.argType = sSelectModel.ARG_TYPE;
            this.o_argType = sSelectModel.ARG_TYPE;
            let s_argId = '4000';
            this.oWpData.target_column = '';
            if(this.oWpData.argType == 'Regression') {
                s_argId = '4001';
            }
            let s_argInfo:any = await this.oComViewerSvc.getArgInfo(s_argId);
            console.log("s_argInfo: ", s_argInfo);
    ...
    async onBtnClick(pEvent: PointerEvent) {
        console.log("this.oWpData.modelInfo : ", this.oWpData.modelInfo);
        let sTmpCom = this.oWpDiagramSvc.getWpNodes().find(sCom => sCom.id == this.oComId);
        let s_param = {
            method: 'MODEL-INFO',
            location: 'workflow',
            MODEL_ID: this.oWpData.learnedModel.MODEL_ID,
            MODEL_IDX: this.oWpData.learnedModel.MODEL_IDX,
            CUSTOM_YN: this.oWpData.learnedModel.CUSTOM_YN,
            PARAMETER: this.oWpData.parameter,
            FRAMEWORK_TYPE: this.oWpData.learnedModel.FRAMEWORK_TYPE
        }
        let s_modelInfo = await this.oWpTransferModelSvc.getModelInfo(s_param).toPromise();
        let s_layerInfo = JSON.parse(s_modelInfo)['data']['evaluateLog']
        console.log("s_layerInfo : ", s_layerInfo);
        let s_data = {
                schema: s_layerInfo,
                code: this.oWpData.pythonCode,
                usetable: '',
                result: {},
                jobId: sTmpCom.jobId,
                excuteFlag: false,
                popupType: "transfer-model",
                param: s_param
            };
        const dialogRef = this.o_dialog.open(WpPythonPopupComponent, {
                width: '1400px',
                data: s_data,
                id: 'wp-python-popup'
              });
