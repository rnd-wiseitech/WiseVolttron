...
import { WpImgClassifyData } from 'projects/wp-server/util/component/transper/wp-image-classify';
 ...
 */
@Component({
    selector: 'image-classify',
    template: '' 
})
export class WpImageClassifyComponent extends WpComponent {
    oWpData: COM_IMG_LABEL_ATT;
    oDialog: MatDialog;
    oTransSvc : TranslateService;
    oWpDiagramSvc: WpDiagramService;
    oMetaSvc: WpMetaService;
    constructor( pTransSvc: TranslateService, pComViewerSvc: WpComponentViewerService, pWpData: WpImgClassifyData, pDiaglog: MatDialog, pWpDiagramSvc: WpDiagramService, pMetaSvc : WpMetaService ) {
    ...
    async onBtnClick(pEvent: any) {
        ...
        let sData: any = {type : 'tag', imageCount : sImageCount, label: this.oWpData.label, tagList: this.oWpData.tagList, currentJobId : sCurrentNode.jobId, usetable: this.oSchema['wp-data']['o_data']['usetable']};
        ...
        const dialogRef = this.oDialog.open(WpImageListPopUpComponent, {
            ...
            if (pRes) {
                this.oWpData.label = Object.entries(pRes.label).map(([pLabelDataKey, pLabelDataValue]:[string, any]) =>{
                    return { fileName : pLabelDataKey, category : pRes.tagList, images: pLabelDataValue['images'], value : pLabelDataValue['label']};
                })
