...
export class WpVolttronService extends WpSeriveImple {
    ...
    getTopicList():Observable<any>{
        return this.cHttp.post(this.cAppConfig.getServerPath('NODE') + '/wkservice/viewVolttronTopicList', {});
    }
    // #119 선택한 토픽의 컬럼 정보 조회
    getTopicSchema(pData:any):Observable<any>{
        return this.cHttp.post(this.cAppConfig.getServerPath('NODE') + '/wkservice/VolttronTopicSelect', pData);
    }
}