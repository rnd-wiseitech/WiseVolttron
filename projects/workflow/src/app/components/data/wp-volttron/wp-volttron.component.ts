...
import { WpVolttronService } from "./wp-volttron.service";
...
import { COM_VOLTTRON_ATT } from "projects/wp-server/wp-type/WP_COM_ATT";
import { WpVolttron } from "projects/wp-server/util/component/data/wp-volttron";
import { MainAppService } from "projects/main/src/app/app.service";
import { TranslateService } from "@ngx-translate/core";


export class WpVolttronComponent extends WpComponent {
    hide = false;
    oWpData: COM_VOLTTRON_ATT; //type override
    public oFormData:WpPropertiesWrap [];
    public oSelectData:any = {};
    public oComViewerSvc:WpComponentViewerService;
    public oWpVolttronSvc:WpVolttronService;
    ...
    constructor(
        ...
        pWpData: WpVolttron,
        pWpVolttronSvc:WpVolttronService,
        ...
        this.oFormData = [
            // {
            //     vname:pTransSvc.instant("WPP_WORKFLOW.COMPONENT.BUTTON.button18"),
            //     name:'filename',
            //     value:'',
            //     type:'button',
            //     fvalue:[],
            //     visible:true,
            //     edit:true,
            //     callbak:this.onVoltronConnectPopup.bind(this)
            // },
            {
        ...
        this.oWpVolttronSvc = pWpVolttronSvc;
    ...
    setFileNameList() {
        this.oWpVolttronSvc.getTopicList().toPromise().then(sTopicList=>{
            console.log(sTopicList)
    ...
    onColChange(pEvent:any){
        this.oComViewerSvc.showProgress(true);
        this.oFormData[0].value = '';
        this.oWpVolttronSvc.getTopicSchema({topic:this.oWpData.topic}).toPromise().then(sResult=>{
            try {
                let sTopicData = JSON.parse(sResult);
                // 선택된 topic 데이터 oSelectData에 할당
                this.oFormData[0].value = this.oWpData.topic;
                this.oSelectData['name'] = this.oWpData.topic;
    ...
    // 볼트론 커넥터 생성 팝업
    onVoltronConnectPopup(pEvent:any){
        const dialogRef = this.oDialog.open(WpPopupComponent, {
            data: {
                'title': this.cTransSvc.instant("WPP_WORKFLOW.COMPONENT.BUTTON.button18"),
                'flag': true,
        ...
        dialogRef.afterClosed().subscribe(result => {
            ...
            if (result && result.data) {
                this.oWpVolttronSvc.createKafkaConnector(result.data).toPromise().then(pResult => {
    ...
    onDbConnect(pData: any, pFormControl: any) {
        ...
        this.oWpVolttronSvc.getDbList(sConnectData).then((sTbList: any) => {
    ...
    getTableNameList(pData: any, pFormControl: any) {
        ...
        this.oWpVolttronSvc.getTbList(sConnectData).then((sTbList:any) => {
            if(sTbList.length==0){
    ...
    getColumnInfo(pData: any, pFormControl: any) {
        ...
        this.oWpVolttronSvc.getColInfo(sConnectData).then(sColInfo => {
