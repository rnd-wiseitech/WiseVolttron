...
export class WpModelManagement {
    ...
    // modelSave2(p_exeResult:WP_MODEL,p_userInfo:WP_SESSION_USER,p_overWriteModelChk:boolean){

    //     let s_wpModel:WP_MODEL_INFO;
    
    //     s_wpModel.modelMstr.ARG_ID = p_exeResult.algorithmInfo.algorithm.ARG_ID;
    //     s_wpModel.modelMstr.DEL_YN = 'N';
    ...
    //         let s_chkModel = await s_metaDb.select('DP_MODEL_MSTR',[],{MODEL_NM: p_exeResult.saveModelName, REG_USER_NO: p_userInfo.USER_NO , DEL_YN: 'N'});
    
    //         let s_modelDatasetUseMstrData:DP_MODEL_DATASET_USE_MSTR_ATT = {
    //             DATASET_TYPE : p_exeResult.importData.dataType,
    //             DATASET_NAME : p_exeResult.importData.dataSetName,
    ...
    //             MODEL_EVAL_TYPE: p_exeResult.algorithmInfo.algorithm.ARG_TYPE,
    //             MODEL_EVAL_RESULT: p_exeResult.pythonUniqueId,
                
    //             // 워크플로우 모델 저장안하는 옵션일 경우 DEL_YN을 Y로 지정
    //             DEL_YN: p_exeResult.wkModelSaveFlag == false ? 'Y' : 'N',
    ...
    //             MODEL_ARG_PARAM: JSON.stringify(p_exeResult.algorithmInfo.parameter),
    ...
    //             let s_originModelMstrData:DP_MODEL_MSTR_ATT = (await s_metaDb.select('DP_MODEL_MSTR',[],{'MODEL_ID' : p_exeResult.MODEL_ID},['MODEL_IDX', 'DESC']))[0];
    //             let s_modelIdx = s_originModelMstrData.MODEL_IDX + 1;
    //             let s_originModelIdx = p_exeResult.MODEL_IDX ? p_exeResult.MODEL_IDX : 1;
    
    //             let s_selectModelDatasetUseMstrData:DP_MODEL_DATASET_USE_MSTR_ATT = (await s_metaDb.select('DP_MODEL_DATASET_USE_MSTR',[],{MODEL_ID : p_exeResult.MODEL_ID,MODEL_IDX:s_originModelIdx },['MODEL_IDX', 'DESC']))[0]            
                
    //             // 기존 정보에서 수정한 정보로 변경 
    //             delete s_selectModelDatasetUseMstrData.DATASET_ID;
    //             s_selectModelDatasetUseMstrData.MODEL_IDX = s_modelIdx;
    //             s_selectModelDatasetUseMstrData.DATASET_TYPE = s_modelDatasetUseMstrData.DATASET_TYPE;
    //             s_selectModelDatasetUseMstrData.DATASET_NAME = s_modelDatasetUseMstrData.DATASET_NAME;
    //             s_selectModelDatasetUseMstrData.DATASET_REF_ID = s_modelDatasetUseMstrData.DATASET_REF_ID;
    
    //             s_modelDatasetUseMstrResult = await s_metaDb.insert('DP_MODEL_DATASET_USE_MSTR',s_selectModelDatasetUseMstrData,false);
    
    //             let s_modelMstrSelectData = await s_metaDb.select('DP_MODEL_MSTR',[],{ 'MODEL_ID': p_exeResult.MODEL_ID, 'MODEL_IDX': s_originModelIdx });
                
    //             s_modelMstrData['MODEL_ID'] = p_exeResult.MODEL_ID;
    //             s_modelMstrData['MODEL_IDX'] = s_modelIdx;
    //             s_modelMstrData['MODEL_USE_DATASET_ID'] = s_modelDatasetUseMstrResult;
                
    //             s_modelMstrData = { ...s_modelMstrData, ...s_modelMstrSelectData };
    ...
    //         // 원래 USE
    //         let s_cleansingList = s_varInfo.filter(pVar => pVar.VAR_IMPORT == 'N');
    //         let s_labelList;
    ...
    //                 let clusterLabel = p_exeResult.algorithmInfo.parameter.filter(pVar => pVar.PARAM_NM == "n_clusters");
    //                 if (clusterLabel.length == 0) {
    //                     clusterLabel = p_exeResult.algorithmInfo.parameter.filter(pVar => pVar.PARAM_NM == "min_samples");
    //                 }
    //                 if (clusterLabel.length == 0) {
    //                     clusterLabel = p_exeResult.algorithmInfo.parameter.filter(pVar => pVar.PARAM_NM == "n_components");
    //                 }
    //                 let clusterLabelNum = Number(clusterLabel[0].PARAM_VALUE);
    ...
    */
    ...
    //         // 1. COM_ID 기준으로 조회한 모델 결과(COM_ID에서 가장 큰 MODEL_ID의 최신 모델)
    //         let s_comIdQuery = `
    //             SELECT T1.MODEL_ID, T1.MODEL_IDX, T1.MODEL_NM, T1.ARG_ID, T1.MODEL_EVAL_TYPE, T1.EXCUTE_RESULT AS MODEL_EXCUTE_RESULT, T2.WF_ID, T2.COM_ID, T3.ARG_NM 
    //             FROM DP_MODEL_MSTR T1
    //             INNER JOIN (SELECT MODEL_ID, MAX(MODEL_IDX) as MODEL_IDX, COM_ID, WF_ID FROM DP_MODEL_WORKFLOW_USE_MSTR
    //                         WHERE MODEL_ID IN (
    //                             SELECT MAX(MODEL_ID) AS MODEL_ID FROM DP_MODEL_WORKFLOW_USE_MSTR
    //                             WHERE COM_ID IN  ${s_comIdString} GROUP BY COM_ID)
    //                         GROUP BY COM_ID ) T2
    //             ON T1.MODEL_ID = T2.MODEL_ID AND T1.MODEL_IDX = T2.MODEL_IDX
    //             INNER JOIN DP_ARG_MSTR T3 ON T1.ARG_ID = T3.ARG_ID`;
    ...
    */
        ...
        return new Promise<WiseReturn>(async(resolve, reject) => {
            try {
                ...
                if (s_saveResult.isSuccess) {
                    // let s_modelWorkflowUseMstrData = {
                    //     WF_ID: pResult.workflowId, COM_ID: pResult.comId,
                    //     MODEL_ID: s_modelId, MODEL_IDX: s_modelIdx,
                    // }
                    // // 기존 모델에 update하는 경우
                    // if (s_overwriteFlag) {
                    //     s_modelWorkflowUseMstrData.MODEL_ID = s_modelWorkflowUseMstrData.MODEL_ID;
                    // }
                    // //DP_MODEL_WORKFLOW_USE_MSTR 업데이트
                    // await this.saveWkUseModel(s_modelWorkflowUseMstrData);
    ...
    // getModelList(p_modelId?:string,p_modelNm?:string,p_storageType?:string){
    //     return new Promise<WiseReturn>((resolve, reject) => {

    //         let s_metaDb = global.WiseMetaDB;
    //         // WP-177 모델 불러올 때 사용자 모델 여부와 torch 모델 여부 갖고 오기
    //         let s_layerModelQuery = `(SELECT ARG_ID, "Y" AS LAYER_YN FROM DP_ARG_PARAM WHERE (PARAM_NM LIKE '%LAYER' OR PARAM_VALUE LIKE '%LAYER%') AND PARAM_NM NOT LIKE '%PYTORCH%' GROUP BY ARG_ID)`
    //         let s_pyTorchModelQuery = `(SELECT ARG_ID, "Y" AS PYTORCH_YN FROM DP_ARG_PARAM WHERE PARAM_NM LIKE '%PYTORCH%' GROUP BY ARG_ID)`
    //         let s_blockModelQuery = `(SELECT ARG_ID, "Y" AS BLOCK_YN FROM DP_ARG_PARAM WHERE PARAM_NM LIKE 'BLOCK_LAYER' GROUP BY ARG_ID)`
    ...
    //         s_sql += this.o_userInfo.USER_MODE == 'ADMIN' ? '' : ` AND B.REG_USER_NO =  ${this.o_userInfo.USER_NO}`;
                    
    //         if (p_storageType != '' && p_storageType != undefined && p_storageType == 'HDFS')
    //             s_sql += " AND C.DATASET_TYPE != 'LOCAL'" ;
                
    //         else 
    //             s_sql += " AND C.DATASET_TYPE != 'HDFS'";
    ...
    //             s_sql += " AND A.MODEL_ID = " + p_modelId;
    //         }

    //         s_sql += " GROUP BY A.MODEL_ID ORDER BY A.MODEL_ID DESC ";
