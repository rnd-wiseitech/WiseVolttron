import { COM_NULL_ATT } from "../../../wp-type/WP_COM_ATT";
 ...
 * @example
  * ```ts
    ...
    * ```
 */
export class WpNullData extends WpComponentProperty {
    constructor(p_data:Array<COM_NULL_ATT>){
        super('/job', p_data, 'transform', 'null');

    }

    hasEmptyValue(){
        let sFlag = false;
        this.o_data.forEach((sCol:COM_NULL_ATT) => {
            if (sCol.type == 'remove'){
    ...
    getSparkParams(p_userInfo: WP_SESSION_USER, p_groupId: string, p_jobId: string, p_parentId: string[], p_data:any) {
        ...
        let sDateExpMap:any = {
            ...
            'yyyymmddHHMMss': moment().format('YYYYMMDDHHmmss'),
            'yyyy-mm-dd HH:MM:ss': moment().format('YYYY-MM-DD HH:mm:ss')
        }
        this.o_data.forEach((sNullCol: COM_NULL_ATT) => {
            let sValue;
            if (sNullCol.dateexp == '') {
                sValue = sNullCol.value;
            } else {
                sValue = sDateExpMap[sNullCol.dateexp];
            }
            sDataArray.push({
                column: sNullCol.column,
                type: sNullCol.type,
                value: sValue
            });
        });
        ...
        (this.o_data as COM_NULL_ATT[]).forEach(s_col => {
