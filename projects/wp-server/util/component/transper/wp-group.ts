 ...
 * @example
  * ```ts
    ...
    * ```
 */
export class WpGroupData extends WpComponentProperty {
    constructor(p_data:COM_GROUP_ATT){
        super('/job', p_data, 'transform', 'group');        
    }
    getColumnInfo(pSchema:any){
        let sDerivedSchema = [];
        let sDerivedColumn = this.o_data.derivedColumn;
        // #25 그룹화 컴포넌트에서 파생된 변수타입으로 지정
        let sTransformValue = this.o_data.groupby[0]['value'];
        let sColType = (sTransformValue==="" || isNaN(Number(sTransformValue)))?'string':'double';
        if ( sDerivedColumn.length > 0){
            sDerivedSchema.push({
                "name" : sDerivedColumn,
                "type" : sColType,
                "nullable" : true,
    ...
    hasEmptyValue(){
        if (super.isEmpty(this.o_data.column) || super.isEmpty(this.o_data.derivedColumn)) {
            return true;
        }
        let sFlag = false;
        this.o_data.groupby.forEach((sCol:any) => {
            if (super.isEmpty(sCol.minRange)||super.isEmpty(sCol.maxRange)||super.isEmpty(sCol.value)){
                sFlag = true;
            }
        });
        return sFlag;
    ...
    getSparkParams(p_userInfo: WP_SESSION_USER, p_groupId: string, p_jobId: string, p_parentId: string[], p_data:any) {
        ...
        let sParam: any = {
            ...
            data : {
                ...
                derivedColumn: this.o_data.derivedColumn,
                column: this.o_data.column,
                dataArray: this.o_data.groupby
            }
        }
        ...
        (this.o_data as COM_GROUP_ATT).groupby.forEach(s_col => {
