 ...
 * @example
  * ```ts
    ...
    * ```
 */
export class WpSliceData extends WpComponentProperty {    
    constructor(p_data:Array<COM_SPLIT_ATT>){
        super('/job', p_data, 'transform', 'filter');        
    }
    ...
    getSparkParams(p_userInfo: WP_SESSION_USER, p_groupId: string, p_jobId: string, p_parentId: string[], p_data:any) {
        ...
        (this.o_data as COM_SPLIT_ATT[]).forEach(s_col => {
