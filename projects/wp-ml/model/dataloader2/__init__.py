...
class WiseDataSet(ABC):
    ...
    def setFeature(self):
        try:
            ...
            self.o_data.drop(columns=p_unFeature, inplace=True)
    ...
    def onSplitTrainTest(self,p_target,p_partitionInfo:PARTITION_ATT):
        ...
        if self.o_partitionInfo.option.value != '' :
            ...
            if s_holdout == 0:
                self.o_x_train, self.o_x_test, self.o_y_train, self.o_y_test = self.o_data, self.o_data, s_targetColValue, s_targetColValue
            # 평가 데이터 분할 비율 로 분할
            else :
                s_splitSize = float(int(s_holdout)/100)
                self.o_x_train, self.o_x_test, self.o_y_train, self.o_y_test = train_test_split(self.o_data.drop(columns=p_target), s_targetColValue, test_size=s_splitSize, random_state=123456)
    ...
    def onSplitTime(self,p_target):
        ...
        self.o_x_train, self.o_x_test, self.o_y_train, self.o_y_test = None 
        ...
        # scale 처리 적용
        self.o_x_train, self.o_y_train = s_trainDf[p_unFeature], s_trainDf[p_target]        
        self.o_x_test, self.o_y_test = s_testDf[p_unFeature], s_testDf[p_target]

        return True
    
class WiseImageSet(ABC):

    def __init__(self, p_data):
        self.data = p_data
        self.dataPath = None
        self.modelsetPath = None
